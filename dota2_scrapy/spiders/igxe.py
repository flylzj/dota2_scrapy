...
class igxe(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for i in items:
            ...
            yield scrapy.Request(api, meta={"item": item, "page_num": page_num, "need_proxy": True, "dont_redirect": True, "api": url}, callback=self.get_sale_price)
    ...
    def get_sale_price(self, response):
        ...
        if response.staus != 200:
            yield scrapy.Request(response.url, meta={"item": item, "page_num": page_num, "need_proxy": True, "dont_redirect": True, "api": response.url}, callback=self.get_sale_price)
        ...
        if data:
            ...
            page_num += 1
            api = "https://www.igxe.cn/product/trade/570/{}?page_no={}".format(item_id, page_num)
            yield scrapy.Request(api, meta={"item": item, "page_num": page_num, "need_proxy": True}, callback=self.get_sale_price)
        else:
